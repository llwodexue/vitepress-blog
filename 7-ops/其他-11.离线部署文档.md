...
> 仅在必要的软件安装步骤通过受控提权执行；服务进程与日常运维使用专用低权限账号。离线包应记录来源、版本、校验和与审批信息。
...
无法访问页面时，不要关闭防火墙。确认监听端口后，只在安全组和主机防火墙中放行所需来源与端口（通常为 80/443），并保留默认拒绝策略。
...
- 解决方案：为静态目录授予 Nginx 工作进程账号所需的只读权限；不要设置 `user root`。目录权限、SELinux/AppArmor 上下文与服务账号应一起核查。
...
- 安装之前需要将 sqlite 包移到 subversion 目录
