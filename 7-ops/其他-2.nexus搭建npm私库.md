...
> Nexus、Java 与 Docker 镜像版本必须按当前官方兼容矩阵选择；本文中的 3.63.0 与 Java 8 仅用于说明当时环境，不应直接复制到新部署。
...
- Windows、Linux 和容器部署方式各有差异，应选择与目标操作系统和运维方式匹配的官方发行包或镜像。
...
**注意**：Nexus 所需 Java 版本随发行版变化。若启动提示 Java 不兼容，应按该版本官方要求安装受支持的 JDK，不要强行使用旧版 Java 8。
...
- `-d`：在 Docker 守护进程运行这个镜像
- `-p`：绑定端口，前面的端口表示宿主机端口，后面的表示容器端口
- `--restart=always`：指定 docker 重新启动容器，当服务器或者 docker 进程重启之后，nexus 容器会在 docker 守护进程启动后由 docker 守护进程启动容器
...
不要通过 root 运行 Nexus 解决权限问题。应为数据目录授予容器运行用户所需的最小权限，并使用持久化卷保存 `nexus-data`。
...
- 如果想免密登录推送可以加上 _auth，加密规则：`user:password -> base64`
