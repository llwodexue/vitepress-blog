...
> 优先安装 Docker Compose V2 插件，并使用 `docker compose` 命令。下面的独立二进制方式适合无法使用包管理器的受控环境；下载后应校验官方发布的校验和。`pip` 安装的 Compose V1 已停止维护，不作为新环境方案。
...
客户端是在 Windows 电脑上，可以删除 frps、frps.toml 文件，之后修改 frpc.toml 文件为如下配置
