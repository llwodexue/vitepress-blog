...
> 本文版本与清单仅用于对应的历史实验环境。离线制品必须固定版本、校验发布方签名或校验和，并先在隔离环境验证；文中所有 IP、账号、密码和 Harbor 凭据均须替换为由密钥系统管理的真实值，不能提交到仓库。
...
1. 下载 KubeKey
   ...
   cd kubekey/
   export KKZONE=cn
   # 从官方 Release 下载固定版本的归档文件，校验后再解压执行
   # 不直接执行网络返回的脚本，避免供应链与版本漂移风险
   ```
...
spec:
  hosts:
  - {name: k8s-master, address: 192.168.220.139, internalAddress: 192.168.220.139, user: deploy, privateKeyPath: /secure/path/id_ed25519, timeout: 120}
