...
登录之后更新 root 密码，并创建 admin 用户，并授权表和远程访问权限，授权完就可以使用 Navicat 进行连接了

```sql
# mysql8.0版本更新密码
-- 用密码管理器生成强密码；不要使用示例凭据，也不要授予 *.* 或开放 '%'
CREATE USER 'app_user'@'10.%' IDENTIFIED BY '<strong-secret>';
GRANT SELECT, INSERT, UPDATE, DELETE ON app_db.* TO 'app_user'@'10.%';
-- MySQL 8 默认认证插件通常无需改为 mysql_native_password

-- MySQL 5.6 及其 `PASSWORD()` 写法仅供维护历史实例；新部署应使用受支持版本并遵循其账户管理语法。
```
...
将解压的目录移动到 /usr/local/mongodb 目录下
...
# 端口号
port=27017 
# 默认只监听本机；确需远程访问时绑定内网地址，并在防火墙/安全组中仅放行应用网段
bind_ip=127.0.0.1
...
```sql
use admin;
db.createUser({ user: 'app_user', pwd: '<strong-secret>', roles: [{ role: 'readWrite', db: 'app_db' }] })

# 验证账号是否授权成功, 1 验证成功，0 验证失败
db.auth('app_user', '<strong-secret>')
```
...
[Service]
Type=forking
# 修改为你的 mongodb 安装目录，与你的 mongodb.conf 配置路径
