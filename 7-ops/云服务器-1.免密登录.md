...
> 生产服务器应使用受限的普通运维账号，不要将 `root` 作为日常 SSH 登录账号。公钥应设置口令并妥善轮换；在确认普通账号可登录后，再在 `sshd_config` 中关闭 root 远程登录和密码认证，并保留一个已验证的会话以免误锁。
...
- 2018年以前的版本中默认使用的是 **RSA-2048** 算法，但是这个密钥长度较短存在潜在安全风险（如抗量子攻击能力弱，使用量子计算机破解可能仅需数小时）
- 2018-2020年转向更安全的 **RSA-4096** 算法，但仍非强制默认
- 2020年之后优先推荐 **Ed25519** 算法（基于椭圆曲线 Curve25519，抗侧信道攻击，密钥更短）
