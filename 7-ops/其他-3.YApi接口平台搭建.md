...
> 本文使用的 YApi tar 包、Node 12 与 MongoDB 3.0.6 均为历史环境，不能作为新部署的版本建议。它们已停止维护且存在已知安全风险；新部署应先确认 YApi 的维护状态与兼容矩阵，使用受支持的运行时和数据库版本。

## 简介

`YApi` 由 `YMFE` 开源，旨在为开发、产品、测试人员提供接口管理服务，帮助团队创建、发布和维护 `API`。
...
- 安装 Node：以下 Node 12 操作仅用于复现旧环境，不应用于联网生产部署。
  ...
  **下载链接：**[Node v12.12.0/ (nodejs.org)](https://nodejs.org/download/release/v12.12.0/)
  ...
  # 查看当前目录
  $ pwd
  /home/node-v12.12.0-linux-x64
  
  # 添加 node 环境变量
  $ vim /etc/profile
  export PATH=$PATH:/home/node-v12.12.0-linux-x64/bin
  # 刷新环境变量
  $ source /etc/profile
  # 检测软链接是否生效
  $ node -v
...
>
  ...
  > $ ln -s /home/node-v12.12.0-linux-x64/bin/npm /usr/bin/npm
  > $ ln -s /home/node-v12.12.0-linux-x64/bin/node /usr/bin/node
  > ```

- 安装 MongoDB：以下 MongoDB 3.0.6 操作仅用于复现旧环境，不应用于联网生产部署。
...
所以打开 [http://127.0.0.1:3030/](http://127.0.0.1:3030/) 或 `[你的 IP 地址]: 3030` 即可进入 YApi

生产环境不要直接将管理端口暴露到公网。应通过反向代理启用 HTTPS、限制访问来源，并为数据库设置认证、最小网络暴露与备份恢复策略。
...
- 安装 Node：以下 Node 12 截图仅描述旧版兼容限制；新环境不要因旧版可视化部署功能降级到已停止维护的 Node 版本。
