...
浏览器在请求资源的时候在 header 里面带上 `accept-encoding: gzip` 的参数。Nginx 在接收 Header 之后，发现如果有这个配置，则发送 gzip 之后的文件，如果没有则发送源文件。浏览器根据 response header 来处理要不要针对返回的文件进行解压缩然后展示
