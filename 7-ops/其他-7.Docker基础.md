...
公有云是最常见的云计算部署类型，公有云资源（例如股务器和存储空间）由第三方云服务提供商拥有和运营，这些资源通过 Internet 提供。在公有云中，所有硬件、软件和其他支持性基础结构均为云提供商所拥有和管理。Microsoft Azure 是公有云的一个示例
...
1. 统一标准
   - 应用构建：语言不同导致构建不同（Java、C++、JavaScript），使用 docker build 打成镜像来解决这个问题（类似 Windows exe）
