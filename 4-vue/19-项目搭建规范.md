...
> 以下原有截图基于 Vue CLI，适用于维护旧项目。新 Vue 项目使用 `create-vue` 创建 Vite 项目，并在创建时明确选择 TypeScript、Router、Pinia、ESLint 与格式化方案。

```bash
npm create vue@latest
```

需要后补依赖时，使用与 Vue 3 对应的稳定版本：
...
状态方案应按状态生命周期选择：组件局部状态留在组件，跨组件或跨路由共享的业务状态使用 Pinia；不要为了统一而将所有状态放进 Store。
...
2. 配置 `.prettierrc` 文件：
    ...
    * trailingComma：多行输入的尾逗号是否添加，设置为 `es5`（在 ES5 支持的多行结构中添加尾逗号）
...
在 `package.json` 中配置 scripts，方便一键格式化：
...
之后执行 `npm run format` 即可对整个项目进行格式化。
...
冲突原因：ESLint 的默认规则与 Prettier 的格式化规则不一致（比如 ESLint 要求单引号但 Prettier 格式化了双引号），导致保存时反复切换。

解决方案：安装以下两个插件，让 ESLint 使用 Prettier 的规则进行校验，并关闭 ESLint 中与 Prettier 冲突的规则。
...
将 `.husky/pre-commit` 中的命令改为 `npx lint-staged`。只检查暂存文件，比每次提交格式化整个仓库更快，也避免意外改写无关文件。
...
1. `git add .` 暂存改动
2. `npx cz`（或 `npm run commit`）交互式编写规范提交信息
3. husky pre-commit 钩子自动运行 ESLint/Prettier 校验
4. husky commit-msg 钩子自动校验提交信息格式
5. 校验通过后提交成功
...
| Prettier | 一键格式化代码 |
| ESLint | 代码质量检测 + Prettier 风格校验 |
| Husky | Git 提交时自动触发 lint 校验 |
