...
- 组件按稳定的界面职责拆分，而不是按文件大小机械拆分；父组件负责组合、数据获取和协调，子组件负责展示或局部交互。
- 父组件通过 props 向下传递数据，子组件通过 emit 向上报告事件。props 是只读输入，子组件不应直接修改。
- 跨多层但局部的依赖可使用 provide/inject；跨页面共享且具有业务生命周期的数据再考虑 Store。不要用全局事件总线代替明确的数据流。
...
按照如上的拆分方式后，我们开发对应的逻辑只需要去对应的组件编写就可以
...
- 如果我们 **不希望组件的根元素继承 attribute**，可以在组件中设置 `inheritAttrs: false`
  ...
  禁用 attribute 继承的常见情况是 **需要将 attribute 应用于根元素以外的其他元素**
...
- 多个根节点的 attribute 如果没有显式地绑定，那么会报警告，我们必须手动指定要绑定到哪一个元素上
...
结构：App.vue -> Home.vue -> HomeContent.vue
...
- 如果想让数据变成响应式的，可以使用响应式的一些 API 来完成这些功能，比如 computed 函数
- 我们在使用 length 的时候需要获取其中的 value，这是因为 computed 返回的是一个 ref 对象，需要取出其中的 value 来使用
...
我们在 App.vue 中使用它们：可以插入普通的内容、HTML 元素、组件元素，都是可以的
...
- 当一个组件被用来渲染数组元素时，我们使用插槽，并且希望插槽能够自定义每项内容的显示方式
...
- 这些 chunk.js 会在 **需要时从服务器加载**，并且运行代码，显示对应的内容
...
如果我们的项目过大了，对于某些组件我们希望通过异步的方式来加载（目的是可以对其进行分包处理），那么 Vue 中给我们提供了一个函数：defineAsyncComponent
...
- `ref` 是访问 DOM 或少量命令式子组件能力的逃生口，不应替代 props / emit 数据流。
- 需要暴露子组件方法时，子组件应通过 `defineExpose` 明确声明公共接口；避免依赖整个组件实例或内部状态。
...
我们可以通过 `$parent` 来访问父组件：

- 这里也可以通过 `$root` 访问根组件，但两者会让组件与层级结构强耦合，应优先改为 props、emit 或 provide/inject。
...
- 在这个过程中的 **某一个阶段**，我们可能会想要 **添加一些属于自己的代码逻辑**（比如组件创建完后就请求一些服务器数据）
