...
> Vuex 4 适用于维护既有 Vuex 项目。新建 Vue 3 项目优先选择 Pinia；本文保留 Vuex 内容，用于理解集中式状态管理与迁移存量代码。
...
- 对于简单状态，确实可以通过 props 传递或 provide/inject 共享
...
在 Vuex 中，mutation 负责同步修改状态；action 负责组织异步流程，并在完成后提交 mutation。action 本身不直接修改 state：
...
- 仅把跨组件、跨路由共享且需要追踪的数据放入 store；输入框开关等局部 UI 状态优先留在组件内。
- state 是数据源，getter 负责派生值，mutation 负责同步写入，action 负责异步编排；组件通过 dispatch 或 commit 发起变化。
- 模块应按业务领域拆分，而不是按 API 类型拆分，避免一个全局 store 承担所有功能。

## 安装 devtool

Vue 其实提供了一个 devtools，方便我们对组件或者 Vuex 进行调试：

- 安装 Vue Devtools 浏览器扩展即可
- 它有两种常见的安装方式：
  - 方式一：通过 Chrome 的商店
  - 方式二：手动下载代码，编译、安装
...
- 应用级共享状态通常由一个 store 实例承载，但并不意味着所有状态都应放入 store
...
- context 是一个和 store 实例具有相同方法和属性的 context 对象
