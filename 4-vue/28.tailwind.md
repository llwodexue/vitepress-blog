...
通过添加前缀修饰符，可以为不同状态应用样式。语法极其直观，格式为 "状态:工具类"
...
**最佳实践警告：谨慎使用`@apply`**。过度使用它会让你回到传统 CSS 的老路，失去工具类带来的部分优势。优先考虑将重复的 UI 封装成组件。
...
整个过程可以概括为"扫描 → 生成 → 注入"三个阶段，细分为六个步骤：
  ...
  return {
    ...
    transform(code, id) {
      if (/\.(vue|svelte|jsx|tsx|html)$/.test(id)) {
        const regex = /class(?:Name)?=["']([^"']+)["']/g // 同时匹配 Vue 的 class 和 React 的 className
        let match
