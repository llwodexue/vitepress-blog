...
- 对 key 的引用是弱引用，不影响垃圾回收器的工作
  ...
  - 只要对应 key 置为 null，其 value 就不存在了，规避内存泄漏的风险
  ...
  - 正因为 WeakMap 对键名引用的对象是弱引用关系，因此 WeakMap 内部成员是会取决于垃圾回收机制有没有执行，运行前后成员个数可能是不一样的，而垃圾回收机制的执行又是不可预测的，因此不可遍历
...
Vue 2 使用 `Object.defineProperty` 劫持 data 属性中的 getter 和 setter，在数据变动时发布消息给订阅者
...
- deleteProperty 作为 Proxy 的捕获器，同样无法在不支持 Proxy 的环境中使用
...
`setupRenderEffect` 这里会调用 effect 函数（当数据发生变化时就会重新执行），函数里面判断组件是否挂载，没有则挂载组件，有则更新组件。第一次没有挂载会走挂载操作
...
如果里面有 children 会调用 `mountChildren` 挂载，最终会把 el 挂载到 container 上
