...
- 不过这些处理完的结果并没有进行部署，所以还有一个概念是持续交付
...
- 将 release 版本交付给运维或项目经理，由他们决定何时部署（可能是手动部署）

持续部署（Continuous Deployment）：

- 是持续交付的更进一步，自动化了整个部署流程
- 代码通过所有测试后直接自动部署到生产环境，无需人工干预
...
自动化流程的核心不是“自动复制文件”，而是可追踪、可验证、可回滚：构建产物应由 CI 生成并保存，部署账号只拥有发布目录的最小权限，失败时必须保留当前线上版本。

![img](https://gitee.com/lilyn/pic/raw/master/lagoulearn-img/008i3skNgy1gtt5buus2kj60wu0lgq7002.jpg)

KubeSphere 是一个以 Kubernetes 为基础的容器管理平台，可以在此平台上借助 Jenkins 完成自动化部署流程，实现完整的 CI/CD 流水线。
...
* 当前 Jenkins 运行时要求 Java 17 或更高版本；构建项目使用的 Node、Java 等工具链可与 Jenkins 运行时分离。
...
* rpm：全称为 **The RPM Package Manager**，是 Linux 下一个软件包管理器
...
Jenkins 默认使用 `jenkins` 用户。不要为了访问目录将其改为 `root`；应创建专用发布目录，并仅授予 `jenkins` 或专用部署账号所需的写入权限。

```bash
sudo install -d -o jenkins -g nginx -m 0750 /srv/mall_cms/releases
```
...
releaseDir="/srv/mall_cms/releases/$BUILD_NUMBER"
install -d "$releaseDir"
cp -a dist/. "$releaseDir/"
ln -sfn "$releaseDir" /srv/mall_cms/current
```

### nginx 安装和配置

我们部署会使用 nginx，所以需要先安装一下 nginx：
...
配置 nginx 访问目录，编辑 `/etc/nginx/nginx.conf`：
...
server {
  ...
  location / {
    root   /srv/mall_cms/current;
    index  index.html index.htm;
  }

  # 配置 API 代理（可选，用于解决前后端跨域问题）
  location /api {
...
访问服务器 IP 即可看到部署的项目。如果无法访问，检查：

- 防火墙是否放行 80 端口
- 安全组是否添加了 80 端口规则
- `/root/mall_cms` 目录下是否有 `index.html`
...
```
开发者 push 代码 → Git 仓库
  → Jenkins 检测到代码变更（轮询 / Webhook）
    → 拉取最新代码
    → npm install 安装依赖
    → npm run build 构建项目
    → 将 dist 文件复制到 nginx 静态目录
      → 用户通过浏览器访问部署好的项目
```

这套流程实现了代码推送后自动构建、自动部署，开发人员只需关注代码本身，其余工作由 Jenkins 自动完成。
