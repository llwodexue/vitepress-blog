...
> 本文配置面向 webpack 5。开发服务器只改善本地开发体验，生产环境仍需由真实服务器、网关或 CDN 提供静态资源、路由回退与代理策略。
...
proxy：在开发服务器中设置反向代理，便于本地前端请求后端接口。

- 比如一个 api 请求是 `http://localhost:8888` 但是本地启动服务器的域名是 `http://localhost:8000`，这时候发送网络请求会出现跨域问题
- 可以将请求先发送到开发服务器，再由开发服务器请求 API；浏览器看到的是同源请求。
- 这不会改变生产环境的跨域策略。上线后仍应由网关或后端正确配置 CORS、认证与访问控制。
...
changeOrigin 会修改代理请求中的 `Host` 请求头：
