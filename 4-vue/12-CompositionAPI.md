...
- ref 会返回一个 **可变的响应式对象**，该对象作为一个 **响应式的引用** 维护着它内部的值（reference）
- 它内部的值是在 ref 的 value 属性中被维护的

这里有两个注意事项：

- 在 **模板中引入 ref 的值** 时，**Vue 会自动帮助我们进行解包操作**，所以我们并 **不需要在模板中通过 ref.value** 的方式来使用
...
在后代组件中可以通过 inject 来注入需要的属性和对应的值，inject 可以传入两个参数：
