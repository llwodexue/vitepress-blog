...
- React 框架本身并没有提供任何动画相关的 API，所以在 React 中使用过渡动画我们需要使用一个 **第三方库：react-transition-group**
...
`<Transition>` 一次只处理一个直接子节点或组件；列表的新增、删除和重排应使用 `<TransitionGroup>`。动画只负责视觉状态，不应承担数据同步或业务流程。
...
首次渲染的时候是没有动画的，如果我们希望给它也添加动画，那么就可以增加另外一个属性 appear
...
- 优先动画化 `transform` 与 `opacity`，避免频繁改动会触发布局或绘制的属性。
- 对系统启用“减少动态效果”的用户，使用 `prefers-reduced-motion` 缩短或禁用非必要动画。
- 进入、离开动画应尽量短，并保证键盘操作和焦点管理不依赖动画结束。
...
- transition 组件给我们提供了很多 JavaScript 钩子，这些钩子可以帮助我们监听动画执行到什么阶段了
...
- 添加 `:css="false"`，也会让 Vue 跳过 CSS 的检测，除了性能略高之外，这可以避免过渡过程中 CSS 规则的影响
...
上面的过渡动画要么是对 **单个节点** 进行操作、要么是 **同一时间渲染多个节点中的一个** 进行操作。如果希望渲染的是一个列表，并且该列表中添加或删除数据也希望有动画执行，这时候我们要使用 `<transition-group>` 组件来完成
