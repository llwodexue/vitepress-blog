...
1. 我们都知道 Vue2.0 代理对象会默认递归，所以在数据层级特别深的情况下，响应的速度会慢
   ...
   - Vue3.0 的代理是在 get() 里面对当前属性进行代理，访问到嵌套对象时才对其进行代理（懒代理），而且还会通过 toProxy 这个 weakMap 将代理过的对象存储起来，防止对象被多次代理，这样我们就判断是否代理过当前对象，如果没有代理过，那就对新对象代理

   - 反之，如果代理过了这个对象，那我们就直接返回代理过的对象
