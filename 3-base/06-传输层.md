...
TCP 面向连接：首先需要建立连接，3 次握手，之后可以通过一条管道来互相发数据。发送完数据要断开连接，要不然服务器会用同一个端口去监听客户端发送过来的数据，这样会占用服务器资源
...
传输层的数据长度 = 网络层的总长度 - 网络层的首部长度 - 传输层数据首部长度
...
- 滑动窗口是动态改变的，随着发送方每次发送 Seq 的时候，接收方都会根据当前机器的执行效率、缓存上限、当前缓存大小得出一个合适的窗口大小，并且随着 Ack 回传到发送方
...
- mss（maximum segment size）：每个段最大的数据部分大小
  ...
  **在建立连接时确定**
  ...
  数据链路层（以太网帧规定 1500 字节） -> 网络层（首部包 20 字节，数据包 1480） -> 传输层（首部段至少 20 字节，数据段至多 1460 字节）
...
- 乘法减少：只要网络出现拥塞，把 ssthresh 减半，与此同时，执行慢开始算法（cwnd 又恢复到初始值）
...
真正的序号是在建立连接时确认的，如果用 1、1025 这样的数很容易暴露出是第几个包
...
- time-wait：表示收到了对方的 FIN 报文，并发送了 ACK 报文，就等 2msl 后即可进入 CLOSED 状态了
  - 如果 FIN-WAIT-1 状态，收到了对方同时带 FIN 标志和 ACK 标志的报文
