...
- 在 2011 年由 IETF 标准化为 [RFC 6455](https://www.rfc-editor.org/rfc/rfc6455)，后由 [RFC 7936](https://www.rfc-editor.org/rfc/rfc7936) 补充规范
...
WebSocket 通常使用 80（`ws://`）、443（`wss://`）端口，但端口可由服务端配置；使用常见端口不等于可以绕过防火墙的安全策略。

生产环境应优先使用 `wss://`，并在握手阶段校验 `Origin`、认证凭据和权限；连接建立后仍需做消息级鉴权与输入校验。
...
> 握手协议中的 SHA-1 仅用于生成 `Sec-WebSocket-Accept`，不是通用密码哈希或完整性保护方案；业务密码和签名不得使用 SHA-1。
>
...
- **客户端-服务端**: 客户端和服务端是分离的，它们可以独立地进化
- **无状态**: 客户端和服务端的通信必须是无状态的，状态应包含在请求里的。也就是说请求里要包含服务端需要的所有的信息，以便服务端可以理解请求并可以创造上下文
- **分层系统**: 就像其它的软件架构一样，REST 也需要分层结构，但是不允许某层直接访问不相邻的层
- **统一接口**: 这里分为4点, 他们是: 资源标识符(URI), 资源的操作(也就是方法Method, HTTP动词), 自描述的响应(可以认为是媒体类型 Media-Type), 以及状态管理(超媒体作为应用状态的引擎 HATEOAS, Hypermedia as the Engine of Application State)
...
- gRPC 通常基于 HTTP/2，支持单向和双向流；头部压缩由 HTTP/2 的 HPACK 提供。是否更高效取决于负载、序列化成本和网络条件，不能仅凭协议名称下结论。
...
> FTP URL 中携带密码会泄露到历史记录、日志和监控中；新系统应使用 SFTP/HTTPS，或通过安全的认证配置传递凭据。FTP 本身不提供机密性，除非明确配置并验证 TLS 扩展。
