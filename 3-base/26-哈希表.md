...
- 在哈希函数分布良好且负载因子受控时，查找、插入和删除的平均时间复杂度为 O(1)；冲突严重时最坏可退化为 O(n)（或由实现使用树结构降低退化风险）。
- 实际性能取决于哈希函数、装载因子、内存局部性和实现，不能笼统断言一定比树快。
- 哈希表相对于树来说编码要容易很多
...
- 计算机中有很多编码方案用数字代替单词字符，就是字符编码，比如 ASCII 编码。a 是 97，b 是 98，z 是 122
...
Java 里是如上计算的，使用到了位运算，但是在 JavaScript 中进行较大数据的位运算时会出问题，所以以下代码实现中还是使用取模
...
5. 以 OpenJDK 8 的 `HashMap` 为例，桶中节点数达到阈值且容量足够大时可能树化；阈值是实现细节，不能当作所有语言或 JDK 版本的通用规则。
...
6. 解决 hash 冲突方法有很多
   ...
   - 链式寻址法，就是把存在 hash 冲突的 key 以单向链表的方式来进行存储，这个在 HashMap 里面有使用到
     ...
     OpenJDK 8 的 `HashMap` 使用链式寻址，并在特定阈值下使用红黑树降低长链表的查找成本；具体阈值应以目标 JDK 源码和文档为准。
