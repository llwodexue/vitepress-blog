...
- 浏览器通常会对同一 origin 的 HTTP/1.1 并发连接数设置上限，Chrome 常见实现为 6；该数字属于客户端策略，不是 HTTP/1.1 标准的固定要求，实际值会随浏览器和网络条件变化。
...
Stalled 可能由连接池上限、连接建立、代理或缓存等因素造成，不能简单归因于“只能有 6 个 TCP 连接”。
...
以前 Akamai 有一个 HTTP/1.1 与 HTTP/2 的对比示例，现在没了，对比图时间差还是很明显的（一个2s一个6s）
...
在 HTTP/1.1 的非流水线请求中，同一个 TCP 连接通常要等待前一个响应后再发送下一个请求；即使启用流水线，也受队头阻塞和部署兼容性限制。
...
而在 HTTP/2 中，同一 TCP 连接可承载多个相互独立的流，交错传输请求和响应，减少应用层队头阻塞；但 TCP 丢包仍可能阻塞整个连接，HTTP/3 才在传输层进一步隔离流。
