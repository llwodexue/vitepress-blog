...
- 不可逆
  ...
  单向散列函数：SHA-256、SHA-3 等。MD5、SHA-1 仅适用于兼容旧系统，不用于安全校验
...
> 哈希不是加密：它不可逆，但弱算法的摘要可被碰撞或字典匹配。密码存储应使用服务端专用慢哈希（如 Argon2id、scrypt 或 bcrypt）并为每个密码生成独立随机盐。
...
- 输入发生微小变化时，摘要通常会发生显著变化（雪崩效应）；摘要长度有限，不能保证不同消息一定得到不同摘要，碰撞始终存在。
...
现代 TLS 使用混合密码系统：握手阶段协商密钥并认证对端，随后用 AEAD 对称算法保护数据。生产环境应使用全站 HTTPS 和 TLS 1.2/1.3，禁用 SSL 及 TLS 1.0/1.1；证书校验失败不能通过“忽略错误”绕过。
