...
- 域名是应用层的可读标识，可能映射到多个 IPv4 / IPv6 地址并随调度变化；网络层设备按 IP 地址转发数据包，因此需要 DNS 等解析机制完成映射。
...
1. 先看自己浏览器缓存有没有对应的 IP 记录，同时还要查询一下主机本地文件里面有没有对应记录，如果有记录就没有必要进行后面的步骤了
...
- DNS 根区有 13 个逻辑根服务器标识。每个标识通过 Anycast 部署在多个物理实例上，实例数量和地址会变化，不应把历史 IP 列表视为固定配置。
