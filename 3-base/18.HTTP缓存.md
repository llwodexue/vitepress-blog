...
- `no-cache`：允许存储，但每次复用前都必须向服务器验证；它不是“不缓存”
...
`max-age=0` 常被用于要求立即重新验证，但它与 `no-cache` 不是可在所有请求、响应和缓存实现中无条件互换的同义词。`must-revalidate` 的作用是禁止缓存对已过期响应进行离线复用；是否需要它取决于资源的陈旧数据是否可接受。

![image-20240321145827373](https://gitee.com/lilyn/pic/raw/master/md-img/image-20240321145827373.png)

### 强制缓存

如果一个 URL 对应的是内容哈希资源，发布后不再覆盖同名文件，就可以设置较长的新鲜期；HTML 和未版本化接口通常应短缓存并配合验证器。缓存策略的边界是资源是否可安全复用，而不是文件类型。
...
`Last-Modified` 的粒度通常是秒，快速连续变更可能无法区分；`ETag` 的生成方式由服务端决定，能表达更精细的版本语义。应按资源的更新模型和服务端成本选择，必要时两者同时提供，而不是把 `Last-Modified` 作为通用首选。
...
- 不能
- 这说明基于元数据的验证器并不等价于内容哈希。是否可接受取决于业务对陈旧内容的容忍度，不能只用“概率低”作为缓存正确性的依据。
...
> 如果 max-age 和 expires 属性都没有，找找头里的 Last-Modified 信息。如果有，缓存的寿命就等于头里面 Date 的值减去 Last-Modified 的值除以 10（注：根据 RFC 2616 其实也就是乘以 10%）

也就是在缺少显式缓存指令时，缓存仍可能使用启发式新鲜度。该行为由实现决定，生产资源应明确发送 `Cache-Control`，不要把启发式缓存当作策略。
