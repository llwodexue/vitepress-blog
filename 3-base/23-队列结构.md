...
- front/peek()：返回队列中第一个元素————最先被添加，也将是最先被移除的元素。队列不做任何变动（不移除元素，只返回元素信息——与 Stack 类的 peek 方法非常类似）
...
- 这个时候某个人（比如班长），在击鼓，鼓声停下的那一刻，花落在谁手里，谁就出来表演节目
...
- 参数：所有参与人的姓名，基于的数字，结果：最终剩下的一人的姓名
