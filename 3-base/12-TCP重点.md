...
- 如果客户端迟迟收不到服务端的 SYN-ACK 报文（第二次握手），就会触发超时重传机制，重传 SYN 报文，而且 **重传的 SYN 报文的序列号都是一样的**；重传间隔和次数由操作系统及配置决定。

- 当客户端超时未收到服务端的 SYN-ACK 报文后会重发 SYN；重发次数由 `tcp_syn_retries` 等内核参数控制，默认值和超时退避会随操作系统、版本及网络配置变化，不应写死为 1 秒或 5 次。
...
当服务端收到客户端第一次握手后，就会回 SYN-ACK 报文给客户端，此时服务端就会进入 `SYN_RCVD` 状态
...
SYN-ACK 报文的最大重传次数由 `tcp_synack_retries` 等内核参数决定；具体默认值以目标内核文档和运行时配置为准。
...
- 但是发送方并不清楚这连续的 ACK2 是接收方收到哪个报文而回复的， 那是选择重传 Seq2 一个报文，还是重传 Seq2 之后已发送的所有报文呢（Seq2、Seq3、Seq4、Seq5、Seq6）？
...
如果一直无脑的发数据给对方，但对方处理不过来，那么就会导致触发重发机制，从而导致网络流量无端地浪费
