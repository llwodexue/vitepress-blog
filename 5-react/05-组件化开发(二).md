...
只要是修改了 App 的 state，所有组件都需要重新 render，进行 diff 算法，性能必然是很低的
...
在 React 的开发模式中，通常情况下不需要、也不建议直接操作DOM元素，但是某些特殊的情况，确实需要获取到 DOM 进行某些操作：
...
function hoc(WrapperComponent) {
  ...
  higherOrderComponent.displayName = 'higherOrder'
...
高阶组件本质就是对传入的组件进行拦截，对原有组件的功能在拦截过程中进行增强
