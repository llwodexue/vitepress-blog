...
> 本文是简化版 Fiber 教学实现，用于展示 render 阶段收集副作用、commit 阶段操作 DOM；不等同于 React 当前实现，也没有覆盖 key、并发优先级、卸载清理和完整异常处理。

接下来我们要做的就是将所有 Fiber 对象存储在数组中，为什么要存在数组中呢？

- 因为 Fiber 算法的第二阶段，我们要循环这个数组统一获取 fiber 对象从而构建真实 DOM 对象，并且要将构建起来的真实 DOM 挂载到页面当中
...
- 当左侧节点中的节点全部构建完成以后，我们开启一个 while 循环去构建其他节点过程时，我们会找到每一个节点**父级 fiber 对象**，这样我们就可以为每一个节点 effects 数组添加 fiber 对象了
...
2. 将 currentExecutedFiber 在 commitAllWork 遍历，操作 fiber 对象
   ...
   有的是文本节点，所以我们是倒序获取 fiber 对象的，也就是从左侧最后一个子节点开始收集的，我们先确定每个 fiber 对象的 effectTag 是什么类型再对其进行相应的操作，如果是 placement 则进行节点追加操作

   ```js
   const commitAllWork = fiber => {
     fiber.effects.forEach(item => {
       if (item.effectTag === 'placement') {
         const parentFiber = findHostParent(item)
         if (parentFiber) {
   ...
   const findHostParent = fiber => {
     let parent = fiber.parent
     while (parent && parent.tag !== 'host_component') {
...
const getTag = vdom => {
  if (typeof vdom.type === 'string') {
    ...
    vdom.type.prototype instanceof Component
  ) {
...
如果是组件，他的子级就不是直接 fiber.props.children 了，而是类组件里面 render 方法返回的内容，所以调用 reconcileChildren 方法的时候需要判断是组件还是普通元素，是组件的话调用 render 方法
...
const commitAllWork = fiber => {
  fiber.effects.forEach(item => {
    if (item.effectTag === 'placement') {
      const parentFiber = findHostParent(item)
      if (item.tag === 'host_component' && parentFiber) {
        parentFiber.stateNode.appendChild(item.stateNode)
      }
    }
  })
...
这两个 JSX 只是将 **Hello React** 替换成 **奥利给** 操作，当 DOM 初始化完成渲染之后，我们去备份旧的 fiber 对象，在两秒钟之后我们又调用了 render 方法，就会去创建新的 fiber 对象了
...
const commitAllWork = fiber => {
  fiber.effects.forEach(item => {
    if (item.effectTag === 'delete') {
      const parentFiber = findHostParent(item)
      if (parentFiber && item.stateNode.parentNode === parentFiber.stateNode) {
