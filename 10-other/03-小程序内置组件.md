...
`getUserInfo` 不应作为新项目获取用户信息的入口。需要头像昵称时，应在合适的交互中使用 `wx.getUserProfile` 并说明用途；手机号事件数据应交给服务端换取手机号，不能把敏感凭据放在小程序端。

![image-20230928160328961](https://gitee.com/lilyn/pic/raw/master/lagoulearn-img/image-20230928160328961.png)

```html
<button size="mini" type="primary" bindtap="getUserProfile">获取头像昵称</button>
