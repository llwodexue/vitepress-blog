...
> 跨端方案应由目标平台、原生能力、性能目标、团队技术栈和长期维护成本共同决定，不能仅按“复杂/不复杂”或单一框架标签选择。
...
- 原生开发适合需要深度平台能力、极致性能或复杂交互的场景，但也可通过成熟跨端框架满足部分需求
- Flutter、uni-app、Taro 等各有运行时、平台覆盖和生态取舍，应先做目标端与关键能力的 PoC 再决策
- 跨端公共层应承载可共享的业务与 UI；平台差异集中在适配层，避免在业务代码中散落条件分支
...
- 命令行工具与模板随 uni-app 版本变化较快，应以当前 uni-app 官方文档为准；旧 Vue CLI 创建方式仅用于维护已有项目。
