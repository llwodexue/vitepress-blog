...
> 性能结论应区分浏览器内部阶段、实验室指标和真实用户体验；具体阈值需按目标设备、核心路径和基线数据制定。
...
TCP 四次挥手发生在连接关闭时，不是页面首次加载的必经步骤；HTTP/2、HTTP/3 还可能复用或采用不同的传输连接。

**URL解析：** UI 线程会判断输入的地址是搜索的关键词还是访问站点的 URL
...
- 建立链接过程中还要看协议，如果是 HTTPS 需要建立 TLS 连接
...
3. 接下来任务交给绘画线程和复合线程来做
   ...
   主线程遍历布局树后会创建绘制记录，因为绘画是有顺序的，之后将页面拆分构建成 **图层树**，最后复合线程把图层创建成一个 **复合帧**
...
- Interaction to Next Paint（INP）和 Largest Contentful Paint（LCP）

`TTI` 已不再是稳定的核心体验指标；可用 LCP、INP、CLS 观察用户体验，用 TBT 等实验室指标定位主线程阻塞。
...
- 资源体积太大
  ...
  使用 Tree-shaking 将没有用到的代码摇下去（基于 ES6 import export）
