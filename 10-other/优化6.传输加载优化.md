...
http {
  ...
  keepalive_requests 100; // 客户端和服务端进行TCP链接后，会开始计数，第101个请求就需要重新建立 TCP链接
}
```
...
- 因为 JS 和 CSS 在 Webpack 里都使用 Hash 命名方式，这也可以保证 HTML 更新到最新，拿到的 JS 和 CSS 也是最新的
...
- 二进制传输
  ...
  HTTP/1.1 使用文本格式；二进制分帧主要改善解析和协议扩展，不直接提供加密或安全性
...
- Server push（历史机制）
  ...
  HTTP/2 定义了服务器推送，但浏览器支持和实际收益有限，现代项目通常使用 `preload`、缓存策略和资源优先级提示代替，不应默认启用
...
自签名证书仅适用于受控本地开发。不要指导用户绕过证书告警；生产环境应使用受信任 CA 签发的证书并验证域名、有效期和私钥保护。
...
> 不建议在新项目配置 `http2_push`。应使用资源预加载、缓存与优先级策略，并通过真实指标验证首屏收益。
