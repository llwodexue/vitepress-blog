...
4. 页面跳转后互相传递数据如何完成？
   ...
   A -> B navigateTo，可以使用url传参

    B -> A navigatorBack，如何传递参数？A 页面在 wx.navigateTo 的 events 参数中注册监听器，B 页面通过 this.getOpenerEventChannel().emit() 触发事件传递数据
...
- app.js -> globalData:{} -> getApp().globalData.name
  ...
  致命的缺点：数据不是响应式的。直接修改 globalData 不会触发页面重新渲染，推荐使用 Vuex/Pinia 或发布订阅模式进行状态管理