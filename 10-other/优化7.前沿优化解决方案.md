...
> “前沿”方案必须以目标浏览器、可访问性和实测收益为前提。示例中的 React/webpack API 可能属于旧版本，接入前先核对当前框架文档。
...
- `display: inline-block`：进行换行会比 `float: left` 要快，但也没有 `display: flex` 快
...
对于那些在当前页面使用的资源可以利用 Preload，而对一些可能在将来某些页面中使用的资源可以利用 Prefetch。从加载优先级上看，Preload 会提升请求优先级，而 Prefetch 会把资源的优先级放到最低，当浏览器空闲时采取加载
...
**服务端渲染（Server Side Rendering）** 可以用于 SEO 和首屏内容，但会增加服务端渲染成本；应结合缓存、流式渲染和真实用户指标评估。静态页面也可以使用**预渲染（Prerendering）**，适合内容相对稳定的路由。

```bash
npm install -D react-snap
```

在 `package.json` 中增加一条 scripts，这里可以通过 npm 一个钩子函数，在 build 完成后，自动触发 postbuild
