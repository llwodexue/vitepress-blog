...
- `Code Splitting`：代码拆分，按需加载
  ...
  当前路径只加载所需资源，其余资源按用户可能访问的路径延迟加载。切分边界应以首屏、复用关系和加载瀑布为依据，避免产生过多小文件。
...
- 内联脚本只用于经过测量确认的关键路径；它会增加 HTML 体积并削弱独立缓存，不存在通用的 1 KB 阈值
...
4. **LCP: Largest Contentful Paint**：主要内容完成呈现的时刻
5. **CLS: Cumulative Layout Shift**：加载过程中的视觉稳定性
6. **INP: Interaction to Next Paint**：用户交互到下一次绘制的响应
7. **Fully Loaded**：工具定义的加载完成点，不等于业务任务已经可完成

`FMP` 已不再作为 Lighthouse 指标；性能诊断应将实验室录制与真实用户数据结合，并区分网络、主线程和布局问题。
...
- 脚本流
  ...
  现代引擎可能在下载、解析和编译阶段并行工作，但具体阈值、线程策略和收益会随浏览器版本、脚本形态和设备变化。将它作为实现背景，不应据此设计固定大小的拆包规则。
...
- 不要依赖额外括号或 `Optimize.js` 这类历史技巧干预解析策略。先通过性能录制确认瓶颈，再从减少首屏代码、延迟非关键路径和控制依赖边界入手。
...
JS 是动态、弱类型语言，写的时候不会声明和强调它变量的类型，但是对于编译器而言，实际上还是需要知道确定的类型，在解析时，它根据自己的推断，会给这些变量赋一个具体的类型，通常管这些类型叫隐藏类型（hidden class），之后所做的优化都是基于隐藏类型进行的
...
- 对于编译器而言，实际上是有类型的
  ...
  JavaScript 是不区分整数、浮点数和双精度，它们都是数字，但是在编译器里会对这个做出精确的区分，如果使数组里面类型发生变化，就会造成额外的开销，效率就不高了
...
类型越具体，编译器能做的优化就越多，如果越通用，能做的优化余地就越少
...
- 嵌套越深消耗越高，节点越多，最后生成的 DOM 树占用内存就越高
