...
> 本文描述的是浏览器渲染管线的常见模型，不应把某个版本的具体时序或耗时当成固定契约；以 Performance 面板和目标设备实测结果为准。
...
- JavaScript：可以通过 JavaScript 实现一些页面上视觉上的变化，例如：添加 DOM 元素、jQuery 实现动画、CSS 动画
...
2. HTML 有很多标签，标签是通过一对尖括号标记出来的，这个尖括号就可以用于识别，就可以把一些字符串理解成有含义的标记，这些标记最终被换成节点对象，放在链形数据结构里，如下图
...
有一些事件触发频率非常高，甚至会超过帧的刷新速率，比如：`scroll`、`touchstart`、`touchmove`、`mousemove`。这些函数触发频率非常高，在 Main 函数里，可以看到事件处理函数消耗比较高，但实际上并没有必要在一帧里处理很多次，比如滚动，并不关心中心过程，只关心最后滚动到哪里，这样就可能会出现卡顿也就是抖动
...
- `requestAnimationFrame` 通常在下一次绘制前回调；回调中的 DOM 读写可能触发样式计算和布局，之后浏览器再绘制并合成。具体顺序由宿主调度。
...
- `requestIdleCallback` 的问题
  ...
  `requestIdleCallback` 只适合可延后的非关键任务，支持和空闲窗口受浏览器、页面可见性与负载影响；应设置 `timeout` 或提供降级方案，不能依赖它完成关键业务。
...
- `requestAnimationFrame` 是在 Layout 和 Paint 之前触发。在这一帧开始渲染之前执行
