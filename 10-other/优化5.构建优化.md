...
> 本文以 Webpack 4/5 的优化思路为背景。新项目应先使用当前构建工具的默认优化，并通过构建产物、覆盖率和真实加载数据定位瓶颈；旧插件只在维护旧链路时参考。

## webpack 优化配置

webpack 4 引入了 mode 模式，可以配置开发和生产模式，就可以使用一些已经默认好的插件帮我们达到想做的效果
...
- `uglifyjs-webpack-plugin`：历史方案，不建议用于新项目；仅用于说明 Webpack 4 以前的兼容背景。
...
- `terser-webpack-plugin`：Webpack 4 时代常见方案；Webpack 5 生产模式通常已内置 Terser，只有需要自定义压缩选项时才显式配置。

  terser 无论从效率还是效果上都比 uglifyjs 好，所以 terser 后面作为生产模式下默认的压缩插件
...
- 应用场景
  ...
  DllPlugin 主要服务于旧的 Webpack 构建链路。现代构建工具通常通过持久化缓存、依赖预构建或更细粒度的代码分割解决重复构建问题，采用前应先测量收益与维护成本。
