...
- `SSL`：HTTPS 使用 TLS 建立安全连接；DevTools 中的名称沿用历史习惯，不应理解为仍在使用已废弃的 SSL 协议

- `Request sent`：请求发送出去

- `Waiting (TTFB)`：从发送请求到收到首个响应字节的等待时间。它反映网络、连接复用、服务端处理和缓存等链路，不等同于“白屏时间”。
...
- `Best Practices`：最佳实践
- `SEO`：对搜索引擎有没有做优化
- `Progressive Web App (PWA)`：渐进式 Web 应用相关检查；离线能力是否可用取决于 Service Worker、缓存策略和具体页面，不是自动具备的属性

**度量（Metrics）**

- `First Contentful Paint`：第一个有内容的绘制出现的时间
- `Speed Index`：衡量可视内容呈现速度的实验室指标；分数需结合设备、网络节流和页面类型比较，没有脱离场景的统一合格线

### Core Web Vitals 与诊断指标

面向真实用户体验，优先关注 LCP（加载）、INP（交互响应）和 CLS（视觉稳定性）；实验室中的 TBT 可帮助定位主线程阻塞，但不能替代真实用户的 INP。当前常用的参考目标是在真实用户数据的第 75 百分位中达到 LCP ≤ 2.5 s、INP ≤ 200 ms、CLS ≤ 0.1；它们用于发现风险，不能替代业务任务完成率。
...
- 帧预算取决于屏幕刷新率：60 Hz 约为 16.7 ms，120 Hz 约为 8.3 ms；脚本、样式、布局和绘制共享预算。不要把固定毫秒数当作所有设备的规则。
...
- 应尽早提供可用内容和交互反馈；具体预算以目标设备、网络和核心用户路径的真实数据设定，而不是统一的 5 秒阈值
...
Windows 10 下打开 `Docker Desktop`，之后拉取镜像
...
window.addEventListener('load', e => {
  ...
  console.log('DOM interactive', domInteractive)
})
```
...
// domInteractive 只描述 DOM 进入可交互状态的时刻，不能替代 INP 或 TTI
// DOM Ready 时间:
