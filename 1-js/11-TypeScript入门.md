...
**接口就是一组规范的集合，你继承我，就得提供我要的东西** 。比如你想去国外，就得提供护照

### 对象类型接口

ts 核心原则之一是对值所具有的结构进行类型检查，有时被称为"鸭式辨型法"（一只鸟看起来像鸭子、游起来像鸭子、叫起来像鸭子，那么它就是鸭子）。ts 里，接口的作用就是为这些类型命名和你的代码或第三方代码定义契约
...
2. in 判断一个属性是否属于某个对象
3. typeof 判断基本类型
4. 创建类型保护函数判断类型（参数是联合类型，函数值是 `参数 is xxx`【类型谓词】）
