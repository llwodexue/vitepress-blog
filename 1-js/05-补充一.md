...
> 函数式风格用于让数据转换和副作用边界更清楚，不会自动带来性能或可测试性。对 I/O、缓存、日志和异步失败，仍要由调用方明确处理。
...
- 纯函数
  ...
  - 该函数不能有语义上可观察的函数副作用，诸如"触发事件"、使输出设备输出、或更改输出值以外对象的内容等（如果参数是引用传递，对参数的更改会影响函数以外的数据，因此不是纯函数）
...
柯里化：把多个参数的函数转换成可以具有任意个参数的函数，可以给函数组合提供细粒度的函数
...
闭包可封装状态或缓存数据。它本身不是内存泄漏；只有不再需要的对象仍被长生命周期引用（例如未清理的监听器、定时器或缓存）才会阻止回收。应在不再使用时取消相应引用或执行清理。
...
每个 agent 都由宿主的 **事件循环** 驱动。宿主安排任务、在任务结束后清空微任务队列，并在合适的时机决定是否渲染；任务来源、渲染时机和 Node.js 的阶段不应由浏览器伪代码推导。
