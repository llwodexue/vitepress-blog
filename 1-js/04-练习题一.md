...
> 本页的事件循环结论以浏览器为例。任务来源、渲染时机和 Node.js 的调度细节由宿主决定；不要把“宏任务”当作 ECMAScript 规范术语，也不要依赖固定的渲染顺序。
...
- **JS 异步编程**
  ...
  JavaScript 是单线程的，一次只能执行一个任务，多任务需要排队，可能会造成代码阻塞，为了避免这个问题，出现了异步编程。一般是通过回调函数、事件发布订阅、Promise 来组织代码，本质上都是通过回调函数实现

- **EventLoop 事件循环**

  EventLoop 是宿主提供的调度机制：在当前调用栈清空后，从不同任务来源中选择后续任务执行，并在任务之间执行微任务检查点。JavaScript 语言本身不规定浏览器的任务队列实现。

- **消息队列**

  消息队列是用来存放宏任务的队列（定时器时间到了的回调函数、Ajax 回调函数）
...
- **微任务**
  ...
  微任务：Promise.then、MutationObserver、`queueMicrotask()` 等。`process.nextTick` 是 Node.js 的专有高优先级队列，不应与浏览器的标准微任务 API 等同。
...
const { Maybe, Container } = require('./support')
let ex4 = function (n) {
  return Maybe.of(n).map(value => Number.parseInt(value, 10))
}
