...
- 可达的标准就是从根出发是否能够被找到
- 根包括全局对象、当前调用栈、模块与宿主保留的引用等；实现细节由运行时决定
...
若删除 `obj.o1`，并删除 `obj.o2.prev`，且没有其他根路径指向 `o1`，它才变为可回收对象。循环引用本身不会阻止基于可达性的现代 GC 回收对象。
...
const user1 = { age: 11 }
const user2 = { age: 22 }
// 即使脚本执行完，因为在全局还是找得到，所以不会被回收
const nameList = [user1.age, user2.age]

function fn() {
  // 在严格模式或 ES 模块中，未声明赋值会抛错；不要依赖隐式全局
  const num1 = 1
  // 局部绑定在函数退出后不再从根可达
  const num2 = 2
}
...
- V8 会解释执行并对热点代码优化编译；具体编译器、GC 阶段和触发时机随 V8 版本、设备及运行参数变化

- 堆上限不是固定常数，受 32/64 位、Node.js 参数和宿主限制影响。排查内存问题应采集实际进程的堆快照与 GC 轨迹，而不是套用历史数字

- V8 使用分代等策略减少停顿，但 GC 仍可能影响交互延迟；业务优化的首要目标是减少不必要的长期引用与峰值分配
...
下列分区与晋升阈值用于理解分代回收，不是稳定 API，也不能据此推导具体版本的内存上限或停顿时间。
...
- 活动对象存储于 From 空间
  ...
  如果要申请空间来使用，首先会所有变量对象都分配到 From 空间
...
- 拷贝过程中可能出现晋升（晋升就是将新生代对象移动至老生代）
  ...
  是否晋升取决于对象存活、空间压力和运行时策略，不能依赖固定轮数或占用率
...
- 使用 Chrome DevTools 的 Performance 与 Memory 面板结合监控；不同浏览器的面板名称和指标会变化
...
- GC 的部分阶段可能暂停 JavaScript，现代运行时也会采用增量或并发工作
- 频繁且过长的 GC 仍可能导致卡顿，应通过录制与堆快照确认，而非只凭内存曲线下结论
- 用户使用中感知应用卡顿
...
- 脱离 DOM 树且不再被 JavaScript 或宿主引用的节点可以被回收

- 分离状态的 DOM 节点

  当前 DOM 节点只是从 DOM 树上脱离了，但是在 JS 代码中还在引用着，这种 DOM 称为分离 DOM

  分离 DOM 不必然是泄漏；只有不再需要却仍被长期引用时才是问题。通过堆快照中的 Retainers 路径确认持有者
  ...
  <body>
    ...
    <script>
      ...
      btn.addEventListener('click', function () {
        ...
        console.log(ul) // console.log 也被引用，记得注释掉
        temEle = ul
  			temEle = null
      })
    </script>
...
字面量通常更直接，也避免了不必要的包装对象；但“必然更快”不是可靠规则。应先保证语义清晰，再用目标环境的真实工作负载进行基准测试。
...
防抖：对于高频操作，我们只希望识别一次点击，可以认为是第一次或者最后一次
...
3. `wait - (now - previous)`
4. 此时如果上述结果是大于 0，就意味着当次的操作是一个高频触发，我们想法让它不要去执行 handle，如果这个结果小于等于 0，就意味着当次不是一个高频操作，那么我们就可以执行 handle
