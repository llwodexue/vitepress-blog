...
- 现代 UI 框架会使用组合、不可变数据等思想，但并不等同于采用完整的函数式编程范式
- 函数式编程可以抛弃 this
- 小而明确、模块边界清晰的函数有利于测试与复用；tree shaking 主要依赖 ES 模块静态结构和副作用标注，并非函数式编程自动带来
- 方便测试、并行处理
...
- 面向对象编程：把现实世界中的事物抽象成程序世界中的类和对象，通过 **封装、继承和多态** 来描述事物之间的联系（对现实世界的事物进行抽象）
...
const DelegatingBlogController = {
  // index 方法和 Views.index 方法调用形式一样
  index(posts) {
    return Views.index(posts)
  }
}
const DirectBlogController = {
...
- 调用时观察 `Call Stack`（调用栈）和 `Scope`（作用域）观察闭包的清除过程
...
console.log(_.reverse([...array])) // reverse 会原地修改传入数组
...
依赖外部可变状态会使输出不稳定；写入外部世界、I/O、时间和随机数等才是常见副作用来源：

- 配置文件
- 数据库
- 获取用户的输入

所有的外部交互都有可能产生副作用，副作用也使得方法通用性下降，不适合扩展和可重用性，同时副作用也会给程序带来安全隐患，给程序带来不确定性，但是副作用不可能完全禁止，尽可能控制它们在可控范围内发生
...
// lodash.map 会传递三个参数 value, index|key, collection
...
- `null` 或 `undefined` 本身不会产生副作用，但会使后续的部分函数无法处理；MayBe 用显式分支表达缺失值
...
在 Functor 中传入 `null` 或 `undefined` 后，直接调用 `toUpperCase()` 会抛错；这属于未处理的缺失值，不等同于副作用。
...
- Either 将成功值与错误信息编码为不同分支，调用方可以显式处理失败路径；它不能自动处理所有异常或副作用
...
- IO 函子中的 `_value` 是一个函数，因为函数是一等公民，这里是把函数作为值来处理
...
Folktale 示例用于理解 Task 抽象。新项目应先使用平台的 `Promise`、`async`/`await` 与 `AbortSignal`；引入函数式库前需确认维护状态和团队成本。

异步任务使用 [folktale](https://folktale.origamitower.com/) 标准的函数式编程库来对 Task 演示

- folktale 和 lodash、ramda 不同的是，他没有提供很多功能函数
- 只提供了一些函数式的操作，例如：compose、curry 等，一些函子 Task、Either、MayBe 等
