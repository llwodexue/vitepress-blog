...
- JS 在执行引擎中维护了一个正在工作的工作表，这里会记录当前正在做的事情，当工作表任务全部清空后，这一轮工作就算结束了
...
- 回调函数是通过 then 方法传递的，如果连续串联多个异步任务也会出现回调函数嵌套的问题

  嵌套使用的方式是使用 Promise 最常见的错误，可以使用 Promise 的 then 方法链式调用的特点解决这个问题，尽可能保证异步任务的扁平化
