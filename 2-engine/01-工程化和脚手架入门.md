...
工程化关注的是可重复交付：将创建、校验、构建、测试和发布串成可追踪流程。工具可以替换，但代码规范、依赖边界、质量门禁和发布回滚策略应稳定存在。
...
- 使用 Git Hooks 在代码提交前检查暂存文件（项目质量、风格）
- lint-staged
- 持续集成
...
`create-vue`、`@angular/cli`、Yeoman 等工具都可根据模板创建项目。Vue CLI 和 Create React App 适合维护既有项目，新项目应优先采用各框架当前推荐的创建方式。
...
- 有时候并不需要生成全新项目，只是想给原有项目增加文件（README、babel），可以用 `Sub Generator`
...
- 不要为解决生成器错误降级到 Node 8。应先用 `yo doctor` 检查环境，确认 Generator 的 Node 版本要求，再升级或更换仍维护的 Generator。
...
`code .` 打开 VS Code，创建 `generators/app/index` 文件
