...
- Grunt 是老牌构建工具，插件生态完善，但是它的工作过程是基于临时文件实现的，所以构建速度较慢（SCSS 编译后就会将结果写入到临时文件，下一个插件读取这个临时文件进行操作，环节越多读取文件次数越多）
- Gulp 是基于内存实现的，对文件的处理都是在内存中实现，速度快一些，默认同时支持多个任务，目前比较流行
...
- 安装 Gulp 同时会自动安装一个叫 gulp-cli 模块
...
1. 回调函数
2. Promise、async 和 await（当前受支持的 Node 版本均支持）
3. Stream 方式
...
操作时会发现 `main.css` 没有内容，这是因为文件读写冲突，一边读一边写
...
3. 在 `node_modules` 找到 `zce-pages/bin/zce-pages.js`，里面就是告诉 gulp 工作时的工作目录和 gulpfile 的工作目录
   ...
   之后执行 `gulp/bin/gulp` 就可以执行 gulpfile 了，完成整个工作流过程
