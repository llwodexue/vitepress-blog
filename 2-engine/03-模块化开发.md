...
AMD（Asynchronous Module Definition）：异步模块定义规范。提供了 `define` 和 `require` 关键字实现模块化的操作
...
- 整合了 `CommonJS` 和 `AMD` 规范的特点专门实现浏览器异步模块化的加载
...
> 以下 Node 8/12 的实验参数属于历史背景。当前 Node 使用 `.mjs`、`.cjs` 或 `package.json` 的 `"type"` 字段确定模块类型，通常不需要 `--experimental-modules`。

1. 使用 `.mjs` 表示 ES Module，或在 `package.json` 中设置 `"type": "module"`。
2. 使用 `.cjs` 明确表示 CommonJS，避免在同一包内混淆模块类型。
...
- **注意： import 不是解构导出对象**
  ...
  使用 `.cjs` 可显式标识 CommonJS；具名导入 CommonJS 的行为依赖静态分析，不应作为稳定接口。
...
为兼容不同 Node 版本，CommonJS 加载 ES Module 时优先使用动态 `import()`；不要依赖 `require()` 对部分 ESM 的版本特定支持：
