...
**注意：** 跨域情况下，浏览器出于安全考虑不让自定义响应头通过 JS 获取（详见： [JS 无法获取响应 header 的 Content-Disposition 字段](https://blog.csdn.net/PGguoqi/article/details/106824957) ）
...
nginx 版本 v1.9.8 后，（加上 ngx_http_slice_module）默认自动支持，可以将 `max_ranges` 设置为 `0` 来取消这个设置。
...
- 可以在 nginx 上加入 `limit_rate 1M` 进行测试，这下基本上速度已经是正常了，并行下载比直接下载快了很多
