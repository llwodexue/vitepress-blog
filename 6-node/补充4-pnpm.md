...
所有项目引用的包都在全局一个地方，如果想要对某个包进行调试，其他项目正好引用，本地运行就会受到影响

**兼容问题**

- **symlink** 即软链接的方式可能会在 windows 存在一些兼容的问题，但是针对这个问题，pnpm 也提供了对应的解决方案：在 win 系统上使用一个叫做 **junctions** 的特性来替代软链接，这个方案在 win 上的兼容性要好于 symlink
